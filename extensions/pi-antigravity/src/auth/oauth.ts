  ...
  return new Promise((resolve, reject) => {
    ...
    server.on("error", (err: NodeJS.ErrnoException) => {
      if (err.code === "EADDRINUSE" || err.code === "EACCES") {
        // Windows can reserve the fixed OAuth callback port. Pi's interactive
        // prompt still supports a verified manual callback URL, so keep the
        // login flow alive when the loopback listener is unavailable.
        resolve({
          server,
          waitForCode: () => new Promise<{ code: string; state: string }>(() => {}),
          cleanup: () => {},
        });
      } else {
        reject(err);
