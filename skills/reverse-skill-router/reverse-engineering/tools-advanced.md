...
> For the complete OLLVM deobfuscation workflow, variant ecosystem, and community tool survey, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).

### d810-ng (IDA) — local first choice

The modern maintained edition of D-810 (Next Generation), integrating the **Z3 SMT** solver and covering multiple variants: OLLVM / Tigress / Hodur (PlugX) / Approov.
...
Installation: clone → copy to the IDA plugins directory → load with Ctrl-Shift-D
Source: https://github.com/w00tzenheimer/d810-ng
```

### obpo-plugin (IDA) — strongest results, cloud plugin

Built on Hex-Rays **microcode** + data-flow tracking + program slicing + hybrid (concolic) execution. Widely recognized by the community as one of the strongest.

```text
- Optimizes decompiler output at the microcode level (does not modify ASM)
- Supports IDA 7.5-7.7 + Hex-Rays, multi-architecture (ARM/ARM64/x86/x64/PPC/MIPS)
- Cloud plugin: target functions are uploaded to obpo-server for processing (core is closed-source, plugin is free)
- ⚠️ Use with caution on sensitive samples (the binary is uploaded to a cloud service)
- Usage: right-click dispatcher → OBPO → Mark and process function
Source: https://github.com/obpo-project/obpo-plugin
```

### ollvm-unflattener — Miasm pure-script

No IDA dependency; based on Miasm symbolic execution, BFS multi-level processing, supports x86/x64 Win/Linux.

```bash
git clone https://github.com/cdong1012/ollvm-unflattener
pip install -r requirements.txt   # miasm, graphviz, keystone-engine
python unflattener -i <input> -o <output> -t <func_addr> -a   # -a auto multi-level
```

### ollvm-breaker (Binary Ninja)

Binary Ninja de-flattening; the repo ships the Android hardened sample `libvdog.so` for hands-on practice. Source: amimo/ollvm-breaker

### DeObfBR — specialized for BR obfuscation

Dedicated to removing Goron/Arkari-style BR (indirect branch) obfuscation. Simple trick: marking the data section read-only partially counters it. Source: Mrack/DeObfBR

### deollvm — ARM64 Unicorn

A fallback for handling ARM64 .so files when IDA is unavailable; based on Unicorn. Source: GeT1t/deollvm
