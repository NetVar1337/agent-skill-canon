---
name: router-reverse-skill-router-docs-generator
description: |
...
- **MUST** read `references/vendor-report-rules.md`: choose `malware` / `apt` or `flavor = null` (vulnerability tasks may add the thin `vuln` overlay); with no flavor, only output the original task template and applicable Base elements — this mode does not force IOC/ATT&CK
