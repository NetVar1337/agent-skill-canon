...
5. Do not call IDA mutating tools (`patch_bytes`, rename, comments, types, variables, data creation, bookmarks) unless the user explicitly asks. Do not execute the target, attach to a live process, or perform write/allocate/free operations on the target unless explicitly requested.
