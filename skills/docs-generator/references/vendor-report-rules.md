...
Principle: **quality over quantity in templates** — only 2 vendor full-text flavors (`malware` / `apt`) + Base common elements + **optional thin overlay** (e.g., `vuln` vulnerability technical analysis). Ordinary reverse engineering, penetration testing, CTF, and JS reports keep their task templates and are not dressed up as malware reports by default; `vuln` is **NOT** a third default full-text flavor.
...
## 4. Incident handling
(Execute only within authorization: first confirm scope and preserve evidence such as samples, memory, process trees, network connections, and logs, then isolate the host; after approval by the responsible party, terminate processes, quarantine/remove files, check hosts/startup items, run a full scan and re-verify. Files must not be deleted before evidence preservation.)
...
> **NOT** a third default vendor full-text flavor; apply only for vulnerability research tasks or on explicit user request.
